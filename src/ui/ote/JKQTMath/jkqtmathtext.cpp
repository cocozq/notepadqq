/*
   ...
   (DKFZ) & IWR, University of Heidelberg
    ...
    double x, double ybrace, double width, double bw, double cubicshrink = 0.5, double cubiccontrolfac = 0.3)
{
    ...
    path.cubicTo(xl1 - bw * cubicshrink,
        ybrace - bw * cubicshrink + bw * cubiccontrolfac,
        xl1 - bw * cubiccontrolfac,
        ybrace,
        xl1,
        ybrace);
    path.lineTo(xl2, ybrace);
    path.cubicTo(xl2 + bw * cubiccontrolfac,
        ybrace,
        (xl2 + xr1) / 2.0,
        ybrace + bw * cubicshrink - bw * cubiccontrolfac,
        (xl2 + xr1) / 2.0,
        ybrace + bw * cubicshrink);
    path.cubicTo((xl2 + xr1) / 2.0,
        ybrace + bw * cubicshrink - bw * cubiccontrolfac,
        xr1 - bw * cubiccontrolfac,
        ybrace,
        xr1,
        ybrace);
    path.lineTo(xr2, ybrace);
    path.cubicTo(xr2 + bw * cubiccontrolfac,
        ybrace,
        xr2 + bw * cubicshrink,
        ybrace - bw * cubicshrink + bw * cubiccontrolfac,
        xr2 + bw * cubicshrink,
...
QFont JKQTmathText::MTenvironment::getFont(JKQTmathText* parent) const
{
    ...
    case MTEsans:
        if (insideMath)
            f.setFamily(parent->get_fontMathSans());
        else
            f.setFamily(parent->get_fontSans());
        break;
    case MTEtypewriter:
        f.setFamily(parent->get_fontTypewriter());
        break;
    case MTEscript:
        f.setFamily(parent->get_fontScript());
        break;
    case MTEroman:
        if (insideMath)
            f.setFamily(parent->get_fontMathRoman());
        else
            f.setFamily(parent->get_fontRoman());
        break;
    case MTEcaligraphic:
        f.setFamily(parent->get_fontCaligraphic());
        break;
    case MTEblackboard:
        f.setFamily(parent->get_fontBlackboard());
        break;
    };
    f.setBold(bold);
    f.setItalic(italic);
...
QString JKQTmathText::MTenvironment::toHtmlStart(MTenvironment defaultEv) const
{
    ...
    if (insideMath) {
        ...
        if (!defaultEv.italic && italic)
            s = s + "font-style: italic; ";
    }
    if (bold && !defaultEv.bold)
        s = s + "font-weight: bold";

    QStringList td;
    if (underlined && !defaultEv.underlined)
        td << "underline";
    if (overline && !defaultEv.overline)
        td << "overline";
    if (strike && !defaultEv.strike)
        td << "line-through";
    if (td.size() > 0)
        s = s + "text-decoration: " + td.join(", ");
    return "<span style=\"" + s + "\">";
}
...
bool JKQTmathText::MTnode::toHtml(
    QString& /*html*/, JKQTmathText::MTenvironment /*currentEv*/, JKQTmathText::MTenvironment /*defaultEv*/)
{
    return false;
}
    ...
    : JKQTmathText::MTnode(parent)
{
    ...
    if (addWhitespace /*&& (this->text.size()>0)*/ && (!this->text[this->text.size() - 1].isSpace()))
        this->text = this->text + " ";
...
void JKQTmathText::MTtextNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
    double& strikeoutPos)
{
    ...
    if (txt.size() > 0) {
        if (txt[0].isSpace() /*&& br.width()<=0*/)
            width = width + fm.boundingRect("I").width();
        if (txt.size() > 1 &&
            txt[txt.size() - 1].isSpace() /*&& (fm.boundingRect("a ").width()==fm.boundingRect("a").width())*/)
            width = width + fm.width("I");
    }
...
bool JKQTmathText::MTtextNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
...
QString JKQTmathText::MTtextNode::textTransform(
    const QString& text, JKQTmathText::MTenvironment currentEv, bool /*forSize*/)
{
    QString txt = text;
...
JKQTmathText::MTinstruction1Node::MTinstruction1Node(
    JKQTmathText* parent, QString name, MTnode* child, const QStringList& parameters)
    : JKQTmathText::MTnode(parent)
{
    this->name = name;
    this->child = child;
...
void JKQTmathText::MTinstruction1Node::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
    double& strikeoutPos)
{
    ...
    if (name == "bf" || name == "textbf" || name == "mathbf")
        ev.bold = true;
    else if (name == "em")
        ev.italic = !ev.italic;
    else if (name == "it" || name == "textit" || name == "mathit")
        ev.italic = true;
    else if (name == "textcolor" || name == "mathcolor" || name == "color")
        ev.color = QColor(parameters.value(0, ev.color.name()));
    else if (name == "equation") {
        ev.italic = true;
        ev.insideMath = true;
    } else if (name == "sc" || name == "textsc" || name == "mathsc")
        ev.smallCaps = true;
    else if (name == "ul" || name == "underline" || name == "underlined")
        ev.underlined = true;
    else if (name == "ol" || name == "overline" || name == "overlined")
        ev.overline = true;
    else if (name == "strike")
        ev.strike = true;
    else if (name == "rm" || name == "textrm") {
        ev.font = JKQTmathText::MTEroman;
    } else if (name == "mathrm" || name == "text" || name == "mbox" || name == "operatorname") {
        ev.font = JKQTmathText::MTEroman;
        ev.italic = false;
    } else if (name == "mat") {
        ev.font = JKQTmathText::MTEroman;
        ev.italic = false;
        ev.bold = true;
    } else if (name == "cal" || name == "textcal" || name == "mathcal") {
        ev.font = JKQTmathText::MTEcaligraphic;
    } else if (name == "bb" || name == "textbb" || name == "mathbb") {
        ev.font = JKQTmathText::MTEblackboard;
    } else if (name == "tt" || name == "texttt" || name == "mathtt") {
        ev.font = JKQTmathText::MTEtypewriter;
    } else if (name == "sf" || name == "textsf" || name == "mathsf") {
        ev.font = JKQTmathText::MTEsans;
    } else if (name == "script" || name == "textscript" || name == "mathscript") {
        ev.font = JKQTmathText::MTEscript;
    } else if (name == "displaystyle") {
        ev.fontSize = ev.fontSize / 0.8;
    } else if (name == "scriptstyle") {
        ev.fontSize = ev.fontSize * 0.8;
...
double JKQTmathText::MTinstruction1Node::draw(
    QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    doDrawBoxes(painter, x, y, currentEv);
    JKQTmathText::MTenvironment ev = currentEv;
    if (name == "bf" || name == "textbf" || name == "mathbf")
    ...
    if (name == "colorbox" || name == "fbox" || name == "boxed") {
        ...
        if (name == "colorbox")
            fcol = QColor(parameters.value(0, ev.color.name()));
        // qDebug()<<"COLOR="<<fcol;
...
bool JKQTmathText::MTinstruction1Node::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
    JKQTmathText::MTenvironment ev = currentEv;
    if (name == "bf" || name == "textbf" || name == "mathbf")
...
void JKQTmathText::MTsubscriptNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
    double& strikeoutPos)
{
    ...
    if (currentEv.italic)
        width = width - double(fm.width(' ')) * parent->get_italic_correction_factor();
}
...
double JKQTmathText::MTsubscriptNode::draw(QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    ...
    double xx = x;
    if (currentEv.italic)
        xx = xx - double(fm.width(' ')) * parent->get_italic_correction_factor();
    return child->draw(painter, xx, y + shift, ev); //+0.5*fm.boundingRect("A").width();
}

bool JKQTmathText::MTsubscriptNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
    html = html + "<sub>";
...
void JKQTmathText::MTsqrtNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
...
bool JKQTmathText::MTsqrtNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
...
void JKQTmathText::MTfracNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
...
double JKQTmathText::MTfracNode::draw(QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    ...
    if (mode == MTFMfrac || mode == MTFMdfrac || mode == MTFMtfrac) {
        ...
        if (l.length() > 0)
            painter.drawLine(l);
        child1->draw(painter,
            x + xw / 2.0 + (width - width1) / 2.0,
            yline - xh * (parent->get_frac_shift_factor()) - descent1,
            ev1);
        child2->draw(painter,
            x + xw / 2.0 + (width - width2) / 2.0,
            yline + xh * (parent->get_frac_shift_factor()) + ascent2,
            ev2);
    } else if (mode == MTFMstackrel) {
        child1->draw(painter,
            x + xw / 2.0 + (width - width1) / 2.0,
    ...
    if (mode == MTFMstackrel)
        return x + width + xw;
    return x + width + xw;
}

bool JKQTmathText::MTfracNode::toHtml(
    QString& /*html*/, JKQTmathText::MTenvironment /*currentEv*/, JKQTmathText::MTenvironment /*defaultEv*/)
{
    bool ok = false;
    ...
    : JKQTmathText::MTnode(parent)
{
    ...
    for (int i = 0; i < children.size(); i++) {
        if (children[i].size() > this->columns)
            this->columns = children[i].size();
    }
...
void JKQTmathText::MTmatrixNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
    double& strikeoutPos)
{
    ...
    for (int i = 0; i < lines; i++) {
        ...
        for (int j = 0; j < children[i].size(); j++) {
            ...
            if (overallHeight1 > rowheight[i])
                rowheight[i] = overallHeight1;
            if (width1 > colwidth[j])
                colwidth[j] = width1;
        }
    }
    ...
    for (int i = 0; i < lines; i++)
        overallHeight = overallHeight + rowheight[i];
...
double JKQTmathText::MTmatrixNode::draw(QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    ...
    for (int i = 0; i < lines; i++) {
        ...
        // baselines[i]=heights[i]=widths[i];
        for (int j = 0; j < children[i].size(); j++) {
            if (children[i].at(j) != NULL)
                children[i].at(j)->getSize(painter, ev1, width1, baselineHeight1, overallHeight1, strikeoutPos);
            ;
            /*widths[i].operator[](j)=width1;
            baselines[i].operator[](j)=baselineHeight;
            heights[i].operator[](j)=overallHeight1;*/
            if (overallHeight1 > rowheight[i])
                rowheight[i] = overallHeight1;
            if (baselineHeight1 > rowascent[i])
                rowascent[i] = baselineHeight1;
            if (width1 > colwidth[j])
                colwidth[j] = width1;
        }
    }
    ...
    for (int i = 0; i < lines; i++) {
        ...
        if (i < lines - 1)
            yy = yy + (rowheight[i] - rowascent[i]) + xw / 2.0 + rowascent[i + 1];
    }

    return x + width;
}

bool JKQTmathText::MTmatrixNode::toHtml(
    QString& /*html*/, JKQTmathText::MTenvironment /*currentEv*/, JKQTmathText::MTenvironment /*defaultEv*/)
{
    return false;
}
...
void JKQTmathText::MTdecoratedNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
...
double JKQTmathText::MTdecoratedNode::draw(QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    ...
    if (ev.italic)
        deltax += 0.1 * fm.boundingRect("A").width();
    QPen pold = painter.pen();
    QPen p = pold;
    p.setColor(ev.color);
    ...
    if (decoration == MTDvec) {
        painter.setPen(p);
        QPolygonF poly;
        poly << QPointF(x + deltax, dpos) << QPointF(xnew + deltax - 0.2 * wc, dpos)
             << QPointF(xnew - 0.4 * wc + deltax, dpos - dheight / 2.0);
        ...
        if (l.length() > 0)
            painter.drawLine(l);
        QPolygonF poly;
        poly << QPointF(xnew + deltax - 0.4 * wc, dpos) << QPointF(xnew + deltax - 0.2 * wc, dpos + dheight / 2.0)
             << QPointF(xnew + deltax - 0.4 * wc, dpos + dheight);
        painter.drawPolyline(poly);
        painter.setPen(pold);
        ...
        painter.setPen(p);
        QPolygonF poly;
        poly << QPointF(x + w / 2.0 - 0.35 * wc + deltax, dpos + dheight / 2.0) << QPointF(x + w / 2.0 + deltax, dpos)
             << QPointF(x + w / 2.0 + 0.35 * wc + deltax, dpos + dheight / 2.0);
        painter.drawPolyline(poly);
        painter.setPen(pold);
...
bool JKQTmathText::MTdecoratedNode::toHtml(
    QString& /*html*/, JKQTmathText::MTenvironment /*currentEv*/, JKQTmathText::MTenvironment /*defaultEv*/)
{
    // QString f;
    // JKQTmathText::MTenvironment ev=currentEv;
...
void JKQTmathText::MTsuperscriptNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
...
double JKQTmathText::MTsuperscriptNode::draw(
    QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    doDrawBoxes(painter, x, y, currentEv);
    JKQTmathText::MTenvironment ev = currentEv;
...
bool JKQTmathText::MTsuperscriptNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
    html = html + "<sup>";
    bool ok = child->toHtml(html, currentEv, defaultEv);
...
JKQTmathText::MTbraceNode::MTbraceNode(
    JKQTmathText* parent, QString openbrace, QString closebrace, MTnode* child, bool showRightBrace)
    : JKQTmathText::MTnode(parent)
{
    this->child = child;
...
void JKQTmathText::MTbraceNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
...
double JKQTmathText::MTbraceNode::draw(QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    ...
    if (openbrace == "(") {
        ...
        path.cubicTo(xnew,
            (y1 + y2) / 2.0 + fabs(y1 - y2) / 6.0,
            xnew,
            (y1 + y2) / 2.0 - fabs(y1 - y2) / 6.0,
            xnew + brace_fraction * bracewidth,
            y2);
        painter.drawPath(path);
    } else if (openbrace == "[") {
        QPainterPath path;
        ...
        if (l.length() > 0)
            painter.drawLine(l);
        l = QLineF(
            xnew + brace_fraction * bracewidth - 1.5 * lw, y1, xnew + brace_fraction * bracewidth - 1.5 * lw, y2);
        if (l.length() > 0)
            painter.drawLine(l);
    } else if (openbrace == "<") {
        QPainterPath path;
    ...
    if (showRightBrace) {
        ...
        if (closebrace == ")") {
            ...
            path.cubicTo(xnew + bracewidth,
                (y1 + y2) / 2.0 + fabs(y1 - y2) / 6.0,
                xnew + bracewidth,
                (y1 + y2) / 2.0 - fabs(y1 - y2) / 6.0,
                xnew + (1.0 - brace_fraction) * bracewidth,
                y2);
            painter.drawPath(path);
        } else if (closebrace == "]") {
            QPainterPath path;
            ...
            if (l.length() > 0)
                painter.drawLine(l);
            l = QLineF(xnew + (1.0 - brace_fraction) * bracewidth + 1.5 * lw,
                y1,
                xnew + (1.0 - brace_fraction) * bracewidth + 1.5 * lw,
                y2);
            if (l.length() > 0)
                painter.drawLine(l);
        } else if (closebrace == ">") {
...
bool JKQTmathText::MTbraceNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
...
void JKQTmathText::MTbraceNode::getBraceWidth(QPainter& /*painter*/,
    ...
    double& braceheight)
{
    ...
    if (openbrace == "{" || closebrace == "}")
        bracewidth = qMax(bracewidth, lw * 3.5);
}
...
void JKQTmathText::MTlistNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
    ...
    for (int i = 0; i < nodes.size(); i++) {
        {
            ...
            // qDebug()<<"i="<<i<<" width="<<width<<" baselineHeight="<<baselineHeight<<"
            // overallHeight="<<overallHeight<<" strikeoutPos="<<strikeoutPos<<"\n"<<"### nodes["<<i<<"] =
            // "<<nodes[i]->getTypeName()<< "   w="<<w1<<" bh="<<bh<<" oh="<<oh<<" sp="<<sp;
        }
        bool doDraw = true;
        ...
        if (dynamic_cast<MTsuperscriptNode*>(nodes[i])) {
            if (i + 1 < nodes.size()) {                             // is there one mor node behind?
                if (dynamic_cast<MTsubscriptNode*>(nodes[i + 1])) { // is this subscript?
                    ...
                    nodes[i]->getSize(painter, currentEv, w1, bh, oh, sp);
                    double shift = parent->get_super_shift_factor() * fm.xHeight() +
                                   (oh - bh); //(overallHeight-baselineHeight)+(oh-bh);
                    if (wasBrace) {
                        shift = baselineHeight - parent->get_super_shift_factor() * tbr.height() + (oh - bh);
                    }

                    // qDebug()<<"super_sub: super: "<<nodes[i]->getTypeName()<<"  w1="<<w1<<" bh"<<bh<<" oh="<<oh<<"
                    // sp="<<sp;
                    ...
                    // qDebug()<<"super_sub:   sub: "<<nodes[i]->getTypeName()<<"  w2="<<w2<<" bh"<<bh<<" oh="<<oh<<"
                    // sp="<<sp;
                    ...
                    // qDebug()<<"### super+sub";
                    // qDebug()<<"### subsupop: super+sub   overallHeight="<<overallHeight<<"
            ...
            if (i + 1 < nodes.size()) {                               // is there one mor node behind?
                if (dynamic_cast<MTsuperscriptNode*>(nodes[i + 1])) { // is this subscript?
                    ...
                    double shift = parent->get_super_shift_factor() * fm.xHeight() +
                                   (oh - bh); //(overallHeight-baselineHeight)+(oh-bh);
                    if (wasBrace) {
                        shift = baselineHeight -
                                parent->get_super_shift_factor() *
                                    parent->getTBR(currentEv.getFont(parent), "M", painter.device()).height() +
                                (oh - bh);
                    }

                    nodes[i]->getSize(painter, currentEv, w1, bh, oh, sp);
                    // qDebug()<<"sub_super:   sub: "<<nodes[i]->getTypeName()<<"  w1="<<w1<<" bh"<<bh<<" oh="<<oh<<"
                    // sp="<<sp;
                    if (shift + oh - bh > overallHeight - baselineHeight) {
                        overallHeight = baselineHeight + shift + (oh - bh);
                    }
                    i++;
                    nodes[i]->getSize(painter, currentEv, w2, bh, oh, sp);
                    // qDebug()<<"sub_super: super: "<<nodes[i]->getTypeName()<<"  w2="<<w2<<" bh"<<bh<<" oh="<<oh<<"
                    // sp="<<sp;
                    if (shift + bh > baselineHeight) {
                        double lheight = overallHeight - baselineHeight;
                        baselineHeight = shift + bh;
                    ...
                    // qDebug()<<"### sub+super";
                    // qDebug()<<"### subsupop: sub+super1   overallHeight="<<overallHeight<<"
                    // baselineHeight="<<baselineHeight;
                }
            }
        } else if (smb) {
            ...
            if (subsupOperations.contains(s)) {
                ...
                if (i + 1 < nodes.size())
                    subn = dynamic_cast<MTsubscriptNode*>(nodes[i + 1]);
                MTsuperscriptNode* supn = NULL;
                if (i + 2 < nodes.size())
                    supn = dynamic_cast<MTsuperscriptNode*>(nodes[i + 2]);
                ...
                if (subn && supn) { // is this subscript and superscript?
                    ...
                    // qDebug()<<"sub_super:   node: "<<nodes[i]->getTypeName()<<"  w1="<<w1<<" bh"<<bh1<<" oh="<<oh1<<"
                    // sp="<<sp1;
                    subn->get_child()->getSize(painter, ev, w2, bh2, oh2, sp2);
                    // qDebug()<<"sub_super:   node: "<<subn->getTypeName()<<"  w2="<<w2<<" bh2"<<bh2<<" oh2="<<oh2<<"
                    // sp2="<<sp2;
                    supn->get_child()->getSize(painter, ev, w3, bh3, oh3, sp3);
                    // qDebug()<<"sub_super:   node: "<<supn->getTypeName()<<"  w3="<<w3<<" bh3"<<bh3<<" oh3="<<oh3<<"
                    // sp3="<<sp3; double d1=oh1-bh1; double d2=oh2-bh2; double d3=oh3-bh3;
                    ...
                    doDraw = false;
                    xnew += w;
                    // qDebug()<<"### subsupop: sub+super2   overallHeight="<<overallHeight<<"
                    // baselineHeight="<<baselineHeight;
                    ...
                    doDraw = false;
                    xnew += w;
                    // qDebug()<<"### subsupop: sub   overallHeight="<<overallHeight<<"
                    // baselineHeight="<<baselineHeight;
                }
            }
        }
        ...
        if (i < nodes.size() && doDraw) {
            ...
            if (dynamic_cast<MTsuperscriptNode*>(nodes[i])) {
                ...
                shift = parent->get_super_shift_factor() * tbr.height() +
                        (oh - bh); //((overallHeight-baselineHeight)+(oh-bh));
                if (wasBrace) {
                    shift = baselineHeight - parent->get_super_shift_factor() * tbr.height() + (oh - bh);
                }
                // qDebug()<<"+++ super: bh="<<bh<<" oh="<<oh<<" shift="<<shift<<" wasBrace="<<wasBrace<<"
                // baselineHeight="<<baselineHeight<<" overallHeight="<<overallHeight<<" fm.ascent="<<fm.ascent();
                ...
                // qDebug()<<"### else:  bh="<<bh<<" baselineHeight="<<baselineHeight<<"   oh="<<oh<<"
        ...
        if (dynamic_cast<MTsuperscriptNode*>(nodes[i])) {
            ...
            if (i + 1 < nodes.size()) {                             // is there one mor node behind?
                if (dynamic_cast<MTsubscriptNode*>(nodes[i + 1])) { // is this subscript?
                    ...
                    double xnew1 =
                        nodes[i]->draw(painter, xnew, ynew + shift - (ccoverallHeight - ccbaselineHeight), currentEv);
                    i++;
            ...
            if (smb) {
                ...
                if (subsupOperations.contains(s)) {
                    ...
                    if (subn && supn) { // is this subscript and superscript?
                        ...
                        i++;
                        // double xnew3=
                        double xn3 = supn->get_child()->draw(
                            painter, xnew + (w - w3) / 2.0, ynew - bh1 - d3 - fm.xHeight() / 4.0, ev);
                        doDraw = false;
                        ...
                        i++;
                        // double xnew2=
                        double xn1 = subn->get_child()->draw(painter, xnew + (w - w2) / 2.0, ynew + bh2 + d1, ev) +
                                     fm.width(" ");
                        doDraw = false;
                        // xnew+=w;
                        xnew = qMax(xn1, xn2);
...
bool JKQTmathText::MTlistNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
    bool ok = true;
    for (int i = 0; i < nodes.size(); i++) {
                ...
                if (this->parent->jkQtMap->charFgreekMap.contains(n)) {
                    ...
                    if (this->parent->jkQtMap->charFsyMap.contains(n)) {
                        ...
                        // else if (n=="oiiint") { symbol=QChar(0x2230); fontFactor=mathFontFactor; yfactor=+0.1; }
                            ...
                            symbol = QString(2, QChar(0x5A));
                            yfactor = +0.1;
                        } else if (n == "iiint") {
                            symbol = QString(3, QChar(0x5A));
                            yfactor = +0.1;
                        } else if (n == "sum") {
                            symbol = QChar(0x58);
                            heightIsAscent = true;
...
QFont JKQTmathText::MTsymbolNode::getFontName(symbolFont f, QFont& fi)
{
    ...
    switch (f) {
    case MTSFgreek:
        fr.setFamily(parent->get_fontGreek());
        break;
    case MTSFsymbol:
        fr.setFamily(parent->get_fontSymbol());
        break;
    case MTSFbraces:
        fr.setFamily(parent->get_fontBraces());
        break;
    case MTSFintegrals:
        fr.setFamily(parent->get_fontIntegrals());
        break;
    case MTSFcaligraphic:
        fr.setFamily(parent->get_fontCaligraphic());
        break;
    case MTSFblackboard:
        fr.setFamily(parent->get_fontBlackboard());
        break;
    default:
        break;
    }
...
void JKQTmathText::MTsymbolNode::getSizeInternal(QPainter& painter,
    JKQTmathText::MTenvironment currentEv,
    double& width,
    double& baselineHeight,
    double& overallHeight,
    double& strikeoutPos)
{
    ...
    width = 0;
    if (currentEv.insideMath)
        width = qMax(parent->getTBR(f, symb, painter.device()).width(),
            parent->getTBR(f, "i", painter.device()).width()); // fm.width(symbol);
    else
        width = fm.boundingRect(symb).width(); // fm.width(symbol);

    width = qMax(fm.width("j"), width);
    if (symb.isEmpty()) {
        width = fm.width("a");
        if (symbolName == "|")
            width = fm.width("1") * 0.8;
        else if (symbolName == "infty")
            width = fm.width("M");
        else if (symbolName == " ")
            width = parent->getTBR(f, "x", painter.device()).width();
        else if (symbolName == ";")
            width = parent->getTBR(f, "x", painter.device()).width() * 0.75;
        else if (symbolName == ":")
            width = parent->getTBR(f, "x", painter.device()).width() * 0.5;
        else if (symbolName == ",")
            width = parent->getTBR(f, "x", painter.device()).width() * 0.25;
        else if (symbolName == "!")
            width = -parent->getTBR(f, "x", painter.device()).width() * 0.25;
        else if (symbolName == "longleftarrow") {
            width = parent->getTBR(f, "X", painter.device()).width() * 3.5;
            symb = "x";
...
double JKQTmathText::MTsymbolNode::draw(QPainter& painter, double x, double y, JKQTmathText::MTenvironment currentEv)
{
    ...
    // std::cout<<"symbol '"<<symbolName.toStdString()<<"' = "<<std::hex<<symbol.at(0).digitValue()<<" in font
    // '"<<f.family().toStdString()<<"' ("<<QFontInfo(f).family().toStdString()<<"):
    ...
    if (!symbol.isEmpty()) {
        ...
        QPainterPath path =
            makeArrow(x + dx, ypos, width, parent->getTBR(f, "M", painter.device()).height() * 0.5, true, false);
        painter.drawPath(path);
    } else if (symbolName == "longrightarrow") {
        double width = parent->getTBR(f, "X", painter.device()).width() * 3.0;
        double dx = parent->getTBR(f, "X", painter.device()).width() * 0.25;
        double ypos = y - parent->getTBR(f, "x", painter.device()).height() / 2.0;
        QPainterPath path =
            makeArrow(x + dx, ypos, width, parent->getTBR(f, "M", painter.device()).height() * 0.5, false, true);
        painter.drawPath(path);
    } else if (symbolName == "Longleftarrow") {
        double width = parent->getTBR(f, "X", painter.device()).width() * 3.0;
        double dx = parent->getTBR(f, "X", painter.device()).width() * 0.25;
        double ypos = y - parent->getTBR(f, "x", painter.device()).height() / 2.0;
        QPainterPath path =
            makeDArrow(x + dx, ypos, width, parent->getTBR(f, "M", painter.device()).height() * 0.5, true, false);
        painter.drawPath(path);
    } else if (symbolName == "Longrightarrow") {
        double width = parent->getTBR(f, "X", painter.device()).width() * 3.0;
        double dx = parent->getTBR(f, "X", painter.device()).width() * 0.25;
        double ypos = y - parent->getTBR(f, "x", painter.device()).height() / 2.0;
        QPainterPath path =
            makeDArrow(x + dx, ypos, width, parent->getTBR(f, "M", painter.device()).height() * 0.5, false, true);
        painter.drawPath(path);
    } else if (symbolName == "longleftrightarrow") {
        double width = parent->getTBR(f, "X", painter.device()).width() * 3.0;
        double dx = parent->getTBR(f, "X", painter.device()).width() * 0.25;
        double ypos = y - parent->getTBR(f, "x", painter.device()).height() / 2.0;
        QPainterPath path =
            makeArrow(x + dx, ypos, width, parent->getTBR(f, "M", painter.device()).height() * 0.5, true, true);
        painter.drawPath(path);
    } else if (symbolName == "Longleftrightarrow") {
        double width = parent->getTBR(f, "X", painter.device()).width() * 3.0;
        double dx = parent->getTBR(f, "X", painter.device()).width() * 0.25;
        double ypos = y - parent->getTBR(f, "x", painter.device()).height() / 2.0;
        QPainterPath path =
            makeDArrow(x + dx, ypos, width, parent->getTBR(f, "M", painter.device()).height() * 0.5, true, true);
        painter.drawPath(path);
...
bool JKQTmathText::MTsymbolNode::toHtml(
    QString& html, JKQTmathText::MTenvironment currentEv, JKQTmathText::MTenvironment defaultEv)
{
    bool ok = true;
    QString s;
    ...
    if (this->parent->jkQtMap->strMathMap.contains(symbolName)) {
        ...
        ev.fontSize *= 1.7;
        s = "&cap;";
    } else if (symbolName == "bigcup") {
        ev.fontSize *= 1.7;
        s = "&cup;";
    } else if (symbolName == "bigvee") {
        ev.fontSize *= 1.7;
        s = "&or;";
    ...
    if (ok)
        html = html + ev.toHtmlStart(defaultEv) + s + ev.toHtmlAfter(defaultEv);
    return ok;
}
...
void JKQTmathText::loadSettings(QSettings& settings, QString group)
{
    ...
    operatorsubsuper_size_factor =
        settings.value(group + "operatorsubsuper_size_factor", operatorsubsuper_size_factor).toDouble();
    mathoperator_width_factor =
        settings.value(group + "mathoperator_width_factor", mathoperator_width_factor).toDouble();
...
void JKQTmathText::saveSettings(QSettings& settings, QString group)
{
    ...
    if (fontColor != def_fontColor)
        settings.setValue(group + "font_color", QColor2String(fontColor));
    ...
    if (fontEncoding != def_fontEncoding) {
        if (fontEncoding == MTFEunicode)
            settings.setValue(group + "font_encoding", "unicode");
        else if (fontEncoding == MTFEwinSymbol)
            settings.setValue(group + "font_encoding", "win_symbol");
    }
}
...
void JKQTmathText::useXITS()
{
    ...
    if (!fdb.families().contains("XITS")) {
        ...
        if (QFile::exists(":/JKQTmathText/fonts/xits-bold.otf")) {
            i = QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/xits-bold.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
        if (QFile::exists(":/JKQTmathText/fonts/xits-bolditalic.otf")) {
            i = QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/xits-bolditalic.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
        if (QFile::exists(":/JKQTmathText/fonts/xits-italic.otf")) {
            i = QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/xits-italic.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
        if (QFile::exists(":/JKQTmathText/fonts/xits-math.otf")) {
            i = QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/xits-math.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
        if (QFile::exists(":/JKQTmathText/fonts/xits-mathbold.otf")) {
            i = QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/xits-mathbold.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
        if (QFile::exists(":/JKQTmathText/fonts/xits-regular.otf")) {
            i = QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/xits-regular.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
        (void)i;
    }
    ...
    // TODO
    // FIXME
    // Debug() << "family: " << XITSfam << XITSMathfam;
...
{
    QFontDatabase fdb;
#ifdef AUTOLOAD_Asana_FONTS
    // qDebug()<<"has XITS: "<<fdb.families().contains("XITS");
    if (!fdb.families().contains("Asana")) {
        // int i;//=-1;
        if (QFile::exists(":/JKQTmathText/fonts/asana-math.otf")) { /*i=*/
            QFontDatabase::addApplicationFont(":/JKQTmathText/fonts/asana-math.otf");
        }
        // qDebug()<<QFontDatabase::applicationFontFamilies(i);
    }
#endif
...
void JKQTmathText::useAnyUnicode(QString timesFont, QString sansFont)
{
    if (!timesFont.isEmpty()) {
        fontMathRoman = timesFont;
        fontRoman = timesFont;
    }
    if (!sansFont.isEmpty()) {
        fontMathSans = sansFont;
        fontSans = sansFont;
    }
    useSTIXfonts = false;
    useXITSfonts = false;
    ...
    if (c == '\\') { // read an instruction name
        ...
        if (c == '|' || c == ';' || c == ':' || c == '!' || c == ',' || c == '_' || c == '\\' || c == '$' || c == '%' ||
            ...
            while (c.isLetter() && (currentTokenID < parseString.size())) {
                currentTokenName += c;
                currentTokenID++;
                if (currentTokenID < parseString.size())
                    c = parseString[currentTokenID];
            }
            if (!c.isLetter())
                currentTokenID--;
        ...
        if (parsingMathEnvironment) {
            ...
            while (c != '(' && c != '&' && c != '[' && c != '|' && c != ')' && c != ']' && c != '\\' && c != '$' &&
                   c != '{' && c != '}' && c != '_' && c != '^' && (currentTokenID < parseString.size())) {
            ...
            while ((!c.isSpace()) && c != '&' && c != '\\' && c != '$' && c != '{' && c != '}' && c != '_' &&
                   c != '^' && (currentTokenID < parseString.size())) {
                // add whitespaces only once
                if (c.isSpace()) {
                    if (!currentTokenName.isEmpty()) {
                        if (!currentTokenName[currentTokenName.size() - 1].isSpace())
...
JKQTmathText::MTnode* JKQTmathText::parseLatexString(bool get, QString quitOnClosingBrace, QString quitOnEnvironmentEnd)
{
    ...
    while (currentToken != MTTnone) {
        ...
        if (currentToken == MTTtext) {
            ...
            if (name == "\\")
                break;  // break on linebrak character
            ...
            if (currentToken == MTTopenbrace) {
                ...
                if (name == "sqrt") {
                    ...
                    if (currentTokenID <= parseString.size() - 1) {
                        ...
                        if (c != '}')
                            error_list.append(
                                tr("error @ ch. %1: \verb{...} not closed by '}'").arg(currentTokenID).arg(name));
                        ;
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMfrac));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                } else if (name == "dfrac") {
                    MTnode* n1 = parseLatexString(true);
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMdfrac));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMtfrac));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMstackrel));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTbraceNode(this, "(", ")", new MTfracNode(this, n1, n2, MTFMstackrel)));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMunderbrace));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMunderset));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMoverbrace));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTopenbrace)
                        n2 = parseLatexString(true);
                    if (n1 && n2)
                        nl->addNode(new MTfracNode(this, n1, n2, MTFMoverset));
                    else
                        error_list.append(tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                              .arg(currentTokenID)
                                              .arg(name));
                    ...
                    if (getToken() == MTTtext) {
                        ...
                        while (currentToken != MTTclosebrace)
                            getToken(); // find closing brace '}' after '\\begin{name'
                        if (envname == "matrix" || envname == "array" || envname == "aligned" || envname == "align" ||
                            envname == "cases" || envname == "pmatrix" || envname == "bmatrix" ||
                            ...
                            while (first || currentToken == MTTampersand ||
                                   (currentToken == MTTinstruction && currentTokenName == "\\")) {
                            ...
                            if (envname == "pmatrix")
                                nl->addNode(new MTbraceNode(this, "(", ")", new MTmatrixNode(this, items)));
                                ...
                                nl->addNode(new MTmatrixNode(this, items));
                            // std::cout<<"  creating matrix-node ... done!\n";
                        } else {
                            error_list.append(
                                tr("error @ ch. %1: unknown environment '%2'").arg(currentTokenID).arg(envname));
                        }
                    } else { // find next '}'
                    ...
                    if (getToken() == MTTtext) {
                        ...
                        if (envname == quitOnEnvironmentEnd) {
                            ...
                            error_list.append(tr("error @ ch. %1: '\\end{%2}' widthout preceding '\\begin{%3}'")
                                                  .arg(currentTokenID)
                                                  .arg(envname)
                                                  .arg(envname));
                        }
                    ...
                    if (name == "textcolor" || name == "mathcolor" || name == "color" || name == "colorbox") {
                        ...
                        if (foundError)
                            error_list.append(
                                tr("error @ ch. %1: expected two arguments in '{' braces after '%2' command")
                                    .arg(currentTokenID)
                                    .arg(name));
                        else
                            nl->addNode(new MTinstruction1Node(this, name, parseLatexString(true), QStringList(col)));
                ...
                if (name == "right") {
                    if (currentToken == MTTtext) {
                        if (currentTokenName.size() > 0) {
                            if (QString(currentTokenName[0]) == quitOnClosingBrace || quitOnClosingBrace == "any" ||
                                QString(currentTokenName[0]) == ".") {
                                // std::cout<<"found \\right '"<<currentTokenName.toStdString()<<"'\n";
                                showRightBrace = (QString(currentTokenName[0]) != ".");
                                // if (!showRightBrace) std::cout<<"don't show right brace
                                // '"<<quitOnClosingBrace.toStdString()<<"' !!!\n";
                                if (quitOnClosingBrace != "any")
                        ...
                        if (quitOnClosingBrace == "~" &&
                            (currentTokenName == "rceil" || QString(currentTokenName[0]) == ".")) {
                            showRightBrace = (QString(currentTokenName[0]) != ".");
                            currentTokenName = currentTokenName.right(currentTokenName.size() - 1);
                            break;
                        } else if (quitOnClosingBrace == "}" &&
                                   (currentTokenName == "}" || QString(currentTokenName[0]) == ".")) {
                            showRightBrace = (QString(currentTokenName[0]) != ".");
                            currentTokenName = currentTokenName.right(currentTokenName.size() - 1);
                            break;
                        } else if (quitOnClosingBrace == "_" &&
                                   (currentTokenName == "rfloor" || QString(currentTokenName[0]) == ".")) {
                            showRightBrace = (QString(currentTokenName[0]) != ".");
                            currentTokenName = currentTokenName.right(currentTokenName.size() - 1);
                            break;
                        } else if (quitOnClosingBrace == "#" &&
                            ...
                            break;
                                   ...
                                   (currentTokenName == "rangle" || QString(currentTokenName[0]) == ".")) {
                            showRightBrace = (QString(currentTokenName[0]) != ".");
                            currentTokenName = currentTokenName.right(currentTokenName.size() - 1);
                            break;
                    ...
                    if (currentToken == MTTtext) {
                        if (currentTokenName.size() > 0) {
                            if (currentTokenName[0] == '(') {
                                currentTokenName = currentTokenName.right(
                                    currentTokenName.size() -
                                    1); // we already used the first character from the text token!
                                nl->addNode(new MTbraceNode(this,
                                    "(",
                                    ")",
                                    parseLatexString(currentTokenName.size() <= 0, ")"),
                                    showRightBrace));
                                ...
                                nl->addNode(new MTbraceNode(this,
                                    "_",
                                    "_",
                                    parseLatexString(currentTokenName.size() <= 0, "_"),
                                    showRightBrace));
                            } else if (currentTokenName[0] == '#') {
                                currentTokenName = currentTokenName.right(currentTokenName.size() - 1);
                                nl->addNode(new MTbraceNode(this,
                        ...
                        currentTokenName = currentTokenName.right(currentTokenName.size() - 1);
                        nl->addNode(new MTbraceNode(
                            this, "{", "}", parseLatexString(currentTokenName.size() <= 0, "}"), showRightBrace));
                    ...
                    if (name.size() == 2) {
                        ...
                        if (n0 == 'v' && n1.isLetter()) {
                            ...
                            nl->addNode(new MTdecoratedNode(
                                this, MTDvec, new MTtextNode(this, QString(n1), false, parsingMathEnvironment)));
                        } else if (n0 == 'c' && n1.isLetter()) {
                            done = true;
                            // std::cout<<"found \\v... command\n";
                            nl->addNode(new MTinstruction1Node(
                                this, "mathcal", new MTtextNode(this, QString(n1), false, parsingMathEnvironment)));
                        }
                    } else if (name.size() == 3) {
                        ...
                        if (n0 == "bb" && n1.isLetter()) {
                            ...
                            // std::cout<<"found \\v... command\n";
                            nl->addNode(new MTinstruction1Node(
                                this, "mathbb", new MTtextNode(this, QString(n1), false, parsingMathEnvironment)));
                        }
                    }
                    if (!done)
                        nl->addNode(new MTsymbolNode(this, name, false)); //, addWhite));
                }
            }
        } else if (currentToken == MTTwhitespace) {
            if (!parsingMathEnvironment)
                nl->addNode(new MTwhitespaceNode(this));
            ...
            if (currentToken == MTTinstruction) {
                ...
                if (currentTokenName.size() <= 1) {
                    ...
                    child2 = new MTtextNode(
                        this, currentTokenName.right(currentTokenName.size() - 1), false, parsingMathEnvironment);
                }
            } else {
                getNew = false;
            }
            if (child != NULL)
                nl->addNode(new MTsubscriptNode(this, child));
            if (child2 != NULL)
                nl->addNode(child2);
            ...
            if (child != NULL)
                nl->addNode(new MTsuperscriptNode(this, child));
            if (child2 != NULL)
                nl->addNode(child2);
        } else if (currentToken == MTTopenbrace) {
...
QRectF JKQTmathText::getTBR(const QFont& fm, const QString& text, QPaintDevice* pd)
{
    ...
    if (pd) {
        ...
        /*for (int i=0; i<tbrs.size(); i++) {
            if (tbrs[i].f==fm && tbrs[i].text==text && (tbrs[i].ldpiX==pd->logicalDpiX() &&
        tbrs[i].ldpiY==pd->logicalDpiY() && tbrs[i].pdpiX==pd->physicalDpiX() && tbrs[i].pdpiY==pd->physicalDpiY())) {
...
bool JKQTmathText::parse(QString text)
{
    ...
    if (parsedNode && parseString == ntext)
        return true;
...
void JKQTmathText::getSizeDetail(
...
void JKQTmathText::draw(QPainter& painter, int flags, QRectF rect, bool drawBoxes)
{
    if (getTree() != NULL) {
        ...
        if ((flags & Qt::AlignRight) != 0)
            x = x + rect.width() - width;
        else if ((flags & Qt::AlignHCenter) != 0)
            x = x + (rect.width() - width) / 2.0;

        // care for vertical align
        if ((flags & Qt::AlignBottom) != 0)
            y = y + rect.height() - overallHeight;
        else if ((flags & Qt::AlignVCenter) != 0)
            y = y + (rect.height() - overallHeight) / 2.0;
...
bool JKQTmathText::MTwhitespaceNode::toHtml(
    QString& html, JKQTmathText::MTenvironment /*currentEv*/, JKQTmathText::MTenvironment /*defaultEv*/)
{
    html = html + "&nbsp;";
    return true;
...
void JKQTMathTextLabel::internalPaint()
{
    ...
    QSizeF size;
    {
        ...
        if (!m_mathText->parse(lastText)) {
            qDebug() << "JKQTMathTextLabel::internalPaint(): parse '" << lastText
                     << "': " << m_mathText->parse(lastText) << "\n  " << m_mathText->get_error_list().join("\n")
                     << "\n\n";
        }

        if (buffer.width() <= 0 || buffer.height() <= 0)
            buffer = QPixmap(1000, 100);
...
JKQTmathText::MTplainTextNode::MTplainTextNode(
    JKQTmathText* parent, QString text, bool addWhitespace, bool stripInnerWhitepace)
    : JKQTmathText::MTtextNode(parent, text, addWhitespace, stripInnerWhitepace)
{
}

QString JKQTmathText::MTplainTextNode::textTransform(
    const QString& text, JKQTmathText::MTenvironment /*currentEv*/, bool /*forSize*/)
{
    return text;
}
