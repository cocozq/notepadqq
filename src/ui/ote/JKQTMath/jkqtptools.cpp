/*
Copyright (c) 2008-2015 Jan W. Krieger (<jan@jkrieger.de>, <j.krieger@dkfz.de>), German Cancer Research Center (DKFZ) &
...
std::string jkqtp_bytestostr(double bytes)
{
    ...
    form = "%.3lf";
    if (fabs(data) >= 1024.0)
        res = jkqtp_format(form, data / 1024.0) + " k";
    if (fabs(data) >= 1024.0 * 1024.0)
        res = jkqtp_format(form, data / (1024.0 * 1024.0)) + " M";
    if (fabs(data) >= 1024.0 * 1024.0 * 1024.0)
        res = jkqtp_format(form, data / (1024.0 * 1024.0 * 1024.0)) + " ";
    if (fabs(data) >= 1024.0 * 1024.0 * 1024.0 * 1024.0)
        res = jkqtp_format(form, data / (1024.0 * 1024.0 * 1024.0 * 1024.0)) + " G";
    if (fabs(data) >= 1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0)
        res = jkqtp_format(form, data / (1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0)) + " T";
    if (fabs(data) >= 1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0)
        res = jkqtp_format(form, data / (1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0)) + " E";
    if (fabs(data) == 0)
        res = "0 ";
    return res + "Bytes";
}
...
{
    if (dataa == 0)
        return jkqtp_floattostr(dataa) + unitname;
    std::string u = "";
    double factor = 1;
...
std::string jkqtp_floattounitstr(double data, int past_comma, bool remove_trail0)
{
    ...
    std::string res = jkqtp_format(form, data);
    if (fabs(data) >= 1e3)
        res = jkqtp_format(form, data / 1e3) + "k";
    if (fabs(data) >= 1e6)
        res = jkqtp_format(form, data / 1e6) + "M";
    if (fabs(data) >= 1e9)
        res = jkqtp_format(form, data / 1e9) + "G";
    if (fabs(data) >= 1e12)
        res = jkqtp_format(form, data / 1e12) + "T";
    if (fabs(data) >= 1e15)
        res = jkqtp_format(form, data / 1e15) + "P";
    if (fabs(data) < 1)
        res = jkqtp_format(form, data / 1e-3) + "m";
    if (fabs(data) < 1e-3)
        res = jkqtp_format(form, data / 1e-6) + "u";
    if (fabs(data) < 1e-6)
        res = jkqtp_format(form, data / 1e-9) + "n";
    if (fabs(data) < 1e-9)
        res = jkqtp_format(form, data / 1e-12) + "f";
    if (fabs(data) == 0)
        res = jkqtp_format(form, data);
    if (remove_trail0) {
        if (data == 0)
            return "0";
        ...
        if (res[i] == '.')
            i--; // remove decimal divider
        return res.erase(i + 1);
    }
    return res;
}

std::string jkqtp_floattolatexstr(
    double data, int past_comma, bool remove_trail0, double belowIsZero, double minNoExponent, double maxNoExponent)
{
    ...
    // std::cout<<"adata="<<adata<<"   log(adata)/log(10)="<<log(adata)/log(10.0)<<"   exp="<<exp<<"   adata/pow(10,
    // exp)="<<adata/pow(10.0, (double)exp)<<"\n";
    ...
    if (v != "1" && v != "10")
        return v + std::string("{\\times}10^{") + jkqtp_inttostr(exp) + "}";
    if (v == "10")
        exp = exp + 1;
    return std::string("10^{") + jkqtp_inttostr(exp) + "}";
}

std::string jkqtp_floattohtmlstr(
    double data, int past_comma, bool remove_trail0, double belowIsZero, double minNoExponent, double maxNoExponent)
{
    std::string result;
    if ((belowIsZero > 0) && (fabs(data) < belowIsZero))
    ...
    // if ((-past_comma<exp) && (exp<past_comma)) result= res;
        ...
        std::string v = jkqtp_floattostr(data / pow(10.0, (double)exp), past_comma, remove_trail0);
        if (v != "1")
            result = v + std::string("&times;10<sup>") + jkqtp_inttostr(exp) + "</sup>";
        else
            result = std::string("10<sup>") + jkqtp_inttostr(exp) + "</sup>";
    }
    // std::cout<<"floattohtmlstr("<<data<<") = '"<<result<<"'\n";
    return result;
}

void plotSymbol(QPaintDevice& paintDevice,
    double x,
    double y,
...
void plotSymbol(JKQTPEnhancedPainter& painter,
    double x,
    double y,
    JKQTPgraphSymbols symbol,
    double symbolSize,
...
QVector<QPointF> draw_ellipse(double x,
    double y,
    double a,
    double b,
    double angle_start,
    ...
    QPointF* x_end)
{
    ...
    for (int i = 1; i < controlPoints; i++) {
        ...
        result.append(xp);
        // std::cout<<"t="<<t/M_PI*180.0<<":  sin(al)="<<sina<<" cos(al)="<<cosa<<" sin(t)="<<sint<<" cos(t)="<<cost<<"
        // a="<<a<<" b="<<b<<": ("<<x+a*cost*cosa-b*sint*sina<<", "<<y+a*cost*sina+b*sint*cosa<<") = ("<<xp.x()<<",
        // "<<xp.y()<<") \n";
        t = t + step;
    }
    if (x_end)
...
{
    switch (pos) {
    case JKQTPCADMcomplete:
        return "all";
    case JKQTPCADMticksAndLabels:
        return "ticks+labels";
    case JKQTPCADMticks:
        return "ticks";
    case JKQTPCADMline:
...
JKQTPCAdrawMode String2JKQTPCAdrawMode(QString pos)
{
    ...
    if (s == "all")
        return JKQTPCADMcomplete;
    if (s == "ticks+labels")
        return JKQTPCADMticksAndLabels;
    if (s == "ticks")
        return JKQTPCADMticks;
    if (s == "line")
        return JKQTPCADMline;
    if (s == "none")
...
JKQTPCAlabelType String2JKQTPCAlabelType(QString pos)
{
    ...
    if (s == "exponent_character")
        return JKQTPCALTexponentCharacter;
    if (s == "exponent")
        return JKQTPCALTexponent;
    if (s == "time")
        return JKQTPCALTtime;
    if (s == "date")
        return JKQTPCALTdate;
    if (s == "datetime")
        return JKQTPCALTdatetime;
    return JKQTPCALTdefault;
}
...
JKQTPlabelPosition String2JKQTPlabelPosition(QString pos)
{
    QString s = pos.trimmed().toLower();
    if (s == "min")
        return JKQTPlabelMin;
    if (s == "max")
        return JKQTPlabelMax;
    if (s == "center")
...
{
    QString s = pos.trimmed().toLower();
    if (s == "outside_bottom" || s == "outsidebottom" || s == "outside_leftbottom" || s == "outsideleftbottom" ||
        s == "olb")
        return JKQTPkeyOutsideLeftBottom;
    if (s == "outside_left" || s == "outsideleft" || s == "outside_topleft" || s == "outsidetopleft" || s == "otl")
        return JKQTPkeyOutsideTopLeft;
    if (s == "outside_right" || s == "outsideright" || s == "outside_topright" || s == "outsidetopright" || s == "otr")
        return JKQTPkeyOutsideTopRight;
    if (s == "outside_top" || s == "outsidetop" || s == "outside_lefttop" || s == "outsidelefttop" || s == "olt")
        return JKQTPkeyOutsideLeftTop;

    if (s == "outside_rightbottom" || s == "outsiderightbottom" || s == "orb")
        return JKQTPkeyOutsideRightBottom;
    if (s == "outside_bottomleft" || s == "outsidebottomleft" || s == "obl")
        return JKQTPkeyOutsideBottomLeft;
    if (s == "outside_bottomright" || s == "outsidebottomright" || s == "obr")
        return JKQTPkeyOutsideBottomRight;
    if (s == "outside_righttop" || s == "outsiderighttop" || s == "ort")
        return JKQTPkeyOutsideRightTop;

    if (s == "inside_bottom" || s == "insidebottom" || s == "inside_bottomright" || s == "insidebottomright" ||
        s == "ibr")
        return JKQTPkeyInsideBottomRight;
    if (s == "inside_top" || s == "insidetop" || s == "inside_left" || s == "insideleft" || s == "inside_topleft" ||
        s == "insidetopleft" || s == "itl")
        return JKQTPkeyInsideTopLeft;
    if (s == "inside_right" || s == "insideright" || s == "inside_topright" || s == "insidetopright" || s == "itr")
        return JKQTPkeyInsideTopRight;
    if (s == "inside_bottomleft" || s == "insidebottomleft" || s == "ibl")
        return JKQTPkeyInsideBottomLeft;
    return JKQTPkeyInsideTopRight;
}
...
{
    switch (pos) {
    case JKQTPkeyLayoutOneColumn:
        return "one_column";
    case JKQTPkeyLayoutOneRow:
        return "one_row";
    case JKQTPkeyLayoutMultiColumn:
        return "multi_column";
    }
    return "";
}
...
JKQTPkeyLayout String2JKQTPkeyLayout(QString pos)
{
    ...
    if (s == "one_row" || s == "onerow")
        return JKQTPkeyLayoutOneRow;
    if (s == "multi_column" || s == "multicolumn" || s == "multi")
        return JKQTPkeyLayoutMultiColumn;
    return JKQTPkeyLayoutOneColumn;
}
...
{
    switch (pos) {
    case JKQTPerrorBoxes:
        return "error_box";
    case JKQTPerrorEllipses:
        return "error_ell";
    case JKQTPerrorLines:
        return "error_lines";
    case JKQTPerrorBars:
        return "error_bars";
    case JKQTPerrorSimpleBars:
        return "error_simplebars";
    case JKQTPerrorPolygons:
        return "error_polygons";
    case JKQTPerrorBarsLines:
        return "error_bars_lines";
    case JKQTPerrorBarsPolygons:
        return "error_bars_polygons";
    case JKQTPerrorSimpleBarsLines:
        return "error_simplebars_lines";
    case JKQTPerrorSimpleBarsPolygons:
        return "error_simplebars_polygons";
    case JKQTPnoError:
        return "error_none";
    }
    return "";
...
JKQTPerrorPlotstyle String2JKQTPerrorPlotstyle(QString pos)
{
    QString s = pos.trimmed().toLower();
    if (s == "error_lines")
        return JKQTPerrorLines;
    if (s == "error_box")
        return JKQTPerrorBoxes;
    if (s == "error_ell")
        return JKQTPerrorEllipses;
    if (s == "error_bars")
        return JKQTPerrorBars;
    if (s == "error_simplebars")
        return JKQTPerrorSimpleBars;
    if (s == "error_polygons")
        return JKQTPerrorPolygons;
    if (s == "error_bars_lines")
        return JKQTPerrorBarsLines;
    if (s == "error_bars_polygons")
        return JKQTPerrorBarsPolygons;
    if (s == "error_simplebars_lines")
        return JKQTPerrorSimpleBarsLines;
    if (s == "error_simplebars_polygons")
        return JKQTPerrorSimpleBarsPolygons;
    return JKQTPnoError;
}
...
QString JKQTPgraphSymbols2String(JKQTPgraphSymbols pos)
{
    ...
    case JKQTPfilledCircle:
        return "symbol_filled_circle";
    case JKQTPrect:
        return "symbol_rect";
    case JKQTPfilledRect:
        return "symbol_filled_rect";
    case JKQTPtriangle:
        return "symbol_triangle";
    case JKQTPfilledTriangle:
        return "symbol_filled_triangle";
    case JKQTPdownTriangle:
        return "symbol_down_triangle";
    case JKQTPfilledDownTriangle:
        return "symbol_filled_down_triangle";
    case JKQTPtarget:
        return "symbol_target";
    case JKQTPstar:
        return "symbol_star";
    case JKQTPfilledStar:
        return "symbol_filled_star";
    case JKQTPdiamond:
        return "symbol_diamond";
    case JKQTPfilledDiamond:
        return "symbol_filled_diamond";
    case JKQTPpentagon:
        return "symbol_pentagon";
    case JKQTPasterisc:
        return "symbol_asterisc";
    case JKQTPfilledPentagon:
        return "symbol_filled_pentagon";
    case JKQTPnoSymbol:
        return "none";
    }
    return "";
...
QString JKQTPgraphSymbols2NameString(JKQTPgraphSymbols pos)
{
    ...
    case JKQTPfilledCircle:
        return QObject::tr("filled circle");
    case JKQTPrect:
        return QObject::tr("rectangle");
    case JKQTPfilledRect:
        return QObject::tr("filled rectangle");
    case JKQTPtriangle:
        return QObject::tr("triangle");
    case JKQTPfilledTriangle:
        return QObject::tr("filled triangle");
    case JKQTPdownTriangle:
        return QObject::tr("down triangle");
    case JKQTPfilledDownTriangle:
        return QObject::tr("filled down triangle");
    case JKQTPstar:
        return QObject::tr("star");
    case JKQTPfilledStar:
        return QObject::tr("filled star");
    case JKQTPdiamond:
        return QObject::tr("diamond");
    case JKQTPfilledDiamond:
        return QObject::tr("filled diamond");
    case JKQTPpentagon:
        return QObject::tr("pentagon");
    case JKQTPfilledPentagon:
        return QObject::tr("filled pentagon");
    case JKQTPtarget:
        return QObject::tr("target");
    case JKQTPasterisc:
        return QObject::tr("asterisc");
    case JKQTPnoSymbol:
...
{
    QString s = pos.trimmed().toLower();
    if (s == "symbol_dot" || s == "dot" || s == ".")
        return JKQTPdot;
    if (s == "symbol_cross" || s == "cross" || s == "x")
        return JKQTPcross;
    if (s == "symbol_plus" || s == "plus" || s == "+")
        return JKQTPplus;
    if (s == "symbol_circle" || s == "circle" || s == "o")
        return JKQTPcircle;
    if (s == "symbol_filled_circle" || s == "filled_circle" || s == "fo")
        return JKQTPfilledCircle;
    if (s == "symbol_rect" || s == "rect" || s == "r")
        return JKQTPrect;
    if (s == "symbol_filled_rect" || s == "filled_rect" || s == "fr")
        return JKQTPfilledRect;
    if (s == "symbol_triangle" || s == "triangle" || s == "^")
        return JKQTPtriangle;
    if (s == "symbol_filled_triangle" || s == "filled_triangle" || s == "f^")
        return JKQTPfilledTriangle;
    if (s == "symbol_down_triangle" || s == "down_triangle" || s == "v")
        return JKQTPdownTriangle;
    if (s == "symbol_filles_down_triangle" || s == "filles_down_triangle" || s == "symbol_filled_down_triangle" ||
        s == "filled_down_triangle" || s == "fv")
        return JKQTPfilledDownTriangle;
    if (s == "symbol_target" || s == "target" || s == "t")
        return JKQTPtarget;
    if (s == "symbol_star" || s == "star" || s == "s")
        return JKQTPstar;
    if (s == "symbol_filled_star" || s == "filled_star" || s == "fs")
        return JKQTPfilledStar;
    if (s == "symbol_diamond" || s == "diamond" || s == "d")
        return JKQTPdiamond;
    if (s == "symbol_filled_diamond" || s == "filled_diamond" || s == "fd")
        return JKQTPfilledDiamond;
    if (s == "symbol_pentagon" || s == "pentagon" || s == "p")
        return JKQTPpentagon;
    if (s == "symbol_filled_pentagon" || s == "filled_pentagon" || s == "fp")
        return JKQTPfilledPentagon;
    if (s == "symbol_asterisc" || s == "asterisc" || s == "*")
        return JKQTPasterisc;
...
JKQTPgraphSymbols JKQTPLinePlotStyleComboBox::getSymbol() const
{
    ...
    if (idx == -2)
        return JKQTPnoSymbol;
    if (idx >= 0 && ok)
        return symbols.value(idx, defaultSymbol);
...
void JKQTPLinePlotStyleComboBox::addSymbol(
    JKQTPgraphSymbols symbol, bool line, const QString& name, const QVariant& data)
{
    QPixmap pix(12, 12);
    pix.fill(Qt::transparent);
    JKQTPEnhancedPainter p;
    ...
    : QTableView(parent)
{
    ...
    addAction(act);
    act = new QAction(QIcon(":/jkqtp_copy16_nohead.png"),
        tr("Copy Selection to clipboard (for Excel ...) without header row/column"),
        this);
    ...
    addAction(act);
    act = new QAction(QIcon(":/jkqtp_copy16_nohead.png"),
        tr("Copy Selection to clipboard (as CSV ...) without header row/column"),
        this);
        ...
        if (row == -1) {
            ...
            for (int col = -1; col < model()->columnCount(); col++) {
                if (col == -1) {
                    ...
                    if (back.color() != QColor(0, 0, 0) && index.data(Qt::BackgroundRole).isValid())
                        style = QString("background: %1;").arg(back.color().name());
                    if (style.isEmpty())
                        html += QString("<td style=\"%1\"><nobr>").arg(fsstyle);
                    else
                        html += QString("<td style=\"%1\"><nobr>").arg(style);
                    if (check.isValid()) {
                        if (check.toInt() != 0) {
                            html += QString("|&times;|&nbsp;&nbsp;");
                        } else {
...
void JKQTPEnhancedTableView::copySelectionToExcel(int copyrole, bool storeHead)
{
    ...
    if (sel.size() == 1) {
        ...
        default:
            txt = QString("\"%1\"").arg(
                vdata.toString().replace('"', "''").replace('\n', "\\n ").replace('\r', "\\r ").replace('\t', " "));
            break;
        }
        ...
        if (storeHead) {
            ...
            for (int c = 0; c < colcnt; c++) {
                hrow.append(QString("\"%1\"").arg(model()
                                                      ->headerData(collist[c], Qt::Horizontal)
                                                      .toString()
                                                      .replace('"', "''")
                                                      .replace('\n', "\\n ")
                                                      .replace('\r', "\\r ")
        ...
        for (int r = 0; r < rowcnt; r++) {
            QStringList row;
            if (storeHead)
                row.append(QString("\"%1\"").arg(model()
                                                     ->headerData(rowlist[r], Qt::Vertical)
                                                     .toString()
                                                     .replace('"', "''")
                                                     .replace('\n', "\\n ")
                                                     .replace('\r', "\\r ")
                                                     .replace('\t', " "))); // vertical header
            for (int c = 0; c < colcnt; c++) {
...
void JKQTPEnhancedTableView::copySelectionToCSV(
    int copyrole, bool storeHead, const QString& separator, const QChar& decimalpoint)
{
    if (!model())
        return;
    if (!selectionModel())
...
void JKQTPEnhancedTableView::print(QPrinter* printer, bool onePageWide, bool onePageHigh)
{
    ...
    if (onePageWide) {
        if (totalWidth > p->pageRect().width())
            scale = p->pageRect().width() / totalWidth;
    }
    if (onePageHigh) {
        if (totalHeight > p->pageRect().height())
            scale = qMin(scale, p->pageRect().height() / totalHeight);
    }
    ...
    { // find number of pages needed
        ...
        if (pageCols.size() > 0 && cols > pageCols.last())
            pageCols << cols;
        if (pageCols.size() == 1)
            pageCols << cols;
        double y = hhh, y0 = hhh;
        ...
        if (pageRows.size() > 0 && rows > pageRows.last())
            pageRows << rows;
        if (pageRows.size() == 1)
            pageRows << rows;
    }
...
void JKQTPEnhancedTableView::paint(QPainter& painter, QRect pageRect)
{
    ...
    if (pageRec.width() == 0 || pageRec.height() == 0)
        pageRec = QRect(QPoint(0, 0), getTotalSize().toSize());
    painter.save();
    ...
    for (int ph = 0; ph < pageRows.size() - 1; ph++) {
        for (int pw = 0; pw < pageCols.size() - 1; pw++) {
            if (page < 0 || page == pageCnt) {
                // qDebug()<<"print page "<<ph<<"/"<<pageRows.size()<<pagesHigh<<"
                // "<<pw<<"/"<<pageCols.size()<<pagesWide; painter.drawPicture(p->pageRect().topLeft(), pic);
                double y = 0;
                if (ph == 0) {
                    y = hhh;
                }
                ...
                for (int r = pageRows[ph]; r < pageRows[ph + 1]; r++) {
                    ...
                    if (pw % pagesWide == 0) {
                        ...
                        // verticalHeader()->itemDelegate()->paint(&painter, option, model()->index(r, c,
                        // QModelIndex()));
                        x = vhw;
                    }
                ...
                if (ph == 0) {
                    ...
                    for (int c = pageCols[pw]; c < pageCols[pw + 1]; c++) {
                        ...
                        painter.setPen(headerPen);
                        painter.drawText(QRect(rec.x() + 4, rec.y() + 4, rec.width() - 8, rec.height() - 8),
...
void JKQTPLinePlotStyleWithSymbolSizeComboBox::refill()
{
    ...
    for (int i = 0; i < symbols.size(); i++) {
        if (symbols[i].symbol != defaultSymbol || symbols[i].symbolSize != defaultSize) {
            addSymbol(symbols[i].symbol,
                symbols[i].line,
                symbols[i].symbolSize,
                JKQTPgraphSymbols2NameString(symbols[i].symbol),
                i);
        }
    }
...
void JKQTPLinePlotStyleWithSymbolSizeComboBox::addSymbol(
    JKQTPgraphSymbols symbol, bool line, double symbolSize, const QString& name, const QVariant& data)
{
    ...
    p.setPen(QColor("blue"));
    if (line)
        p.drawLine(QLineF(0, double(pixSize) / 2.0, pixSize, double(pixSize) / 2.0));
    p.end();
    addItem(QIcon(pix), name, data);
}

bool JKQTPLinePlotStyleWithSymbolSizeComboBox::styleData::operator==(
    const JKQTPLinePlotStyleWithSymbolSizeComboBox::styleData& other) const
{
...
static const struct RGBData {
    const char* name;
    unsigned int value;
} rgbTbl[] = {{"aliceblue", rgb(240, 248, 255)},
...
std::string jkqtp_to_valid_variable_name(std::string input)
{
    ...
    for (size_t i = 0; i < input.size(); i++) {
        ...
        if ((isdigit(input[i])) && (out.size() > 0))
            out = out + input[i];
        if ((input[i] == '_') && (out.size() > 0))
            out = out + input[i];
    }
    return out;
}
...
{
#if QT_VERSION >= 0x040800
    qDebug() << this->indent << "TIMER_MESSAGE:  " << this->message << " " << message
             << "    DUR: " << double(nsecsElapsed()) / 1.0e6 << "ms";
#else
    qDebug() << this->indent << "TIMER_MESSAGE:  " << this->message << " " << message
             << "    DUR: " << double(elapsed()) / 1.0e3 << "ms";
#endif
}
